...
3. ChatGPT API編  
    ...
    APIの出力（finish_reason）  
    |出力|意味|
    |:---|:---|
    |stop|APIが完全な結果を返したとき|
    |length|APIが何らかの理由で不完全な結果を返したとき|
    |content_filter|結果が省略されたとき|
    |null|APIの応答が進行中で不完全なとき|
    ...
    VPSサーバー上で、LINEからメッセージを受け取り、それをChatGPTに投げ、結果を LINEに返すPythonプログラムを実行する    
...
4. 大規模言語モデルと作るアプリ開発  
    ...
    GPTシリーズの中にCodexと呼ばれるプログラム生成専用のモデルが存在する  
...
5. 大規模言語モデルを10倍強化する  
    ...
    様々な分野の知識を有し、人間同等の知性と認識能力、感性、思考回路を持つ人工知能（概念上のもので実現していない）  
    汎用人工知能を目標としたLLMと他のツール（検索エンジン等）を組み合わせを「自立駆動型AIエージェント」と呼ぶ  
    ...
    プロンプト入力した文章を判断し、内容に応じて必要な情報を抽出してJSONで返す機能  
